  ...
  return <div data-grudge-command className="fixed bottom-10 left-3 z-[100]">
    {!open ? <button aria-label="Ask Grudge to act in the app" className="flex items-center gap-2 rounded-lg border border-gold/40 bg-bg-2 px-3 py-2 text-sm text-gold shadow-lg" onClick={() => setOpen(true)}><Bot size={16}/>Ask Grudge</button> :
      <section aria-label="App-wide prompt" className="w-[min(450px,calc(100vw-24px))] rounded-xl border border-gold/40 bg-bg-2 p-4 text-fg shadow-2xl">
        ...
        <p className="my-2 text-xs text-muted">{route === "/skeleton" ? "Describe changes to the model open here. Smooth its skin, fit or adjust its skeleton, paint it, or add an idle animation. Each change is saved as a new revision." : "Describe what to do in this app. Grudge uses the current screen and its existing controls."}</p>
        <textarea aria-label="App action prompt" className="min-h-24 w-full rounded border border-line bg-bg p-2 text-sm" maxLength={1800} disabled={busy} value={prompt} onChange={e => setPrompt(e.target.value)} placeholder={route === "/skeleton" ? "Smooth and unify the skin, or move Tail2 bone 5 cm down." : "Describe the next change or app action."}/>
