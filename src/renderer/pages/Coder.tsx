...
import {
  ...
  Bot, Globe, Code2, Zap, ChevronRight, Monitor,
} from "lucide-react";
...
type CoderMode = "production" | "local";

const DEFAULT_PORT = 5111;
const PROD_URL = STUDIO_MODULE_URLS.coder;
...
export default function Coder() {
  ...
  const [mode, setMode] = useWorkspaceField("coderMode", "production" as CoderMode);
  ...
  return (
    <div className={"coder-page" + (isProd ? " coder-page--prod" : "")}>
      <header className="coder-hero">
        <div className="coder-hero-text">
          ...
          <p className="muted text-sm max-w-xl">
            Grudge Studio module — agentic IDE at{" "}
            <span className="font-mono text-gold">coder.grudge-studio.com</span>.
            Production embeds here by default; switch to Local to run GrudachainCode.
            Press <span className="kbd">Ctrl+/</span> for GRUDA Chain.
          </p>
        </div>
        <div className="coder-hero-actions">
          <div className="forge-mode-toggle" role="tablist" aria-label="Coder mode">
            <button
              type="button"
              role="tab"
              aria-selected={isProd}
              className={"forge-mode-btn" + (isProd ? " active" : "")}
      ...
      <div className="coder-ai-strip">
        ...
        <div className={`coder-ai-pill ${isProd ? "ok" : running ? "ok" : ""}`}>
          <Terminal size={12} />
          <span>{isProd ? "Production IDE" : "Coder server"}</span>
          <span className="coder-ai-detail">
      ...
      {isProd ? (
        <div className="coder-prod-wrap">
          ...
          <div className="coder-prod-body">
            {prodLoading && (
              <div className="forge-webview-loading">
                <Loader2 size={22} className="animate-spin text-gold" />
                <span className="text-xs text-muted">Loading Coder…</span>
              </div>
            )}
            <webview
              key={prodKey}
                ...
                {running ? (
                  ...
                  <>
                    ...
                    <p className="muted text-sm max-w-md text-center mb-4">
                      ...
                      Or switch to <strong className="text-gold">Production</strong> to use the hosted IDE with no setup.
                    </p>
