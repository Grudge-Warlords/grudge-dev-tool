...
export interface LoadModelOptions {
  /** Base URL (directory, trailing slash) for relative texture/material paths. */
  resourceBaseUrl?: string;
}

function createLoadingManager(resourceBaseUrl?: string): THREE.LoadingManager | undefined {
  if (!resourceBaseUrl) return undefined;
  const base = resourceBaseUrl.endsWith("/") ? resourceBaseUrl : `${resourceBaseUrl}/`;
  ...
  // Missing external textures (e.g. source PSDs not shipped with the model) are common.
  manager.onError = () => {};
  return manager;
}

/** Load a model from a Blob/File using the right loader for its extension. */
export async function loadModel(file: File, options: LoadModelOptions = {}): Promise<LoadedModel> {
  const format = detectFormat(file.name);
  if (!format) throw new Error(`Unsupported format: ${file.name}`);
  const url = URL.createObjectURL(file);
  const manager = createLoadingManager(options.resourceBaseUrl);
  try {
    switch (format) {
      case "glb":
      case "gltf": {
...
/** Load a model directly from a CDN or file URL (no local File blob). */
export async function loadRemoteModel(url: string, options: LoadModelOptions = {}): Promise<LoadedModel> {
  const format = detectFormat(url);
  if (!format) throw new Error(`Unsupported remote format: ${url}`);
  const manager = createLoadingManager(options.resourceBaseUrl);
  switch (format) {
    case "glb":
    case "gltf": {
    ...
    case "fbx": {
      ...
      return {
        object: fbx,
        animations: (fbx as THREE.Object3D & { animations?: THREE.AnimationClip[] }).animations ?? [],
        gltf: null,
        format,
        ...stats,
      };
    ...
    default:
      throw new Error(`Remote load not implemented for .${format}`);
  }
}
