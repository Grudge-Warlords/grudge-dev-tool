...
import { FLEET_GAMES, STORE_CATEGORIES, mergeFleetGames } from "../shared/fleetGames";
  ...
  app.whenReady().then(async () => {
    ...
    if (mainWindow) {
      mainWindow.webContents.on("before-input-event", (event, input) => {
        ...
        if (ctrlOrCmd && k === "/") { mainWindow!.webContents.send("grudachain:toggle"); event.preventDefault(); }
...
function registerIpc() {
  ...
  function dialogParent(): BrowserWindow | undefined {
    ...
    if (lw && !lw.isDestroyed()) return lw;
    return undefined;
  }

  ipcMain.handle("files:pickForUpload", async () => {
    const r = await dialog.showOpenDialog(dialogParent() ?? (undefined as any), {
      properties: ["openFile", "multiSelections"],
    });
    return r.canceled ? [] : r.filePaths;
  });

  ipcMain.handle("files:pickDirectory", async (_e, opts?: { title?: string }) => {
    const r = await dialog.showOpenDialog(dialogParent() ?? (undefined as any), {
  ...
  ipcMain.handle("files:readBytes", async (_e, path: string) => {
    const data = await readFile(path);
    return { bytes: Array.from(data), size: data.byteLength, name: basename(path) };
  });
  ...
  ipcMain.handle("forge:openPath", (_e, path: string) => {
    if (!path || typeof path !== "string") throw new Error("forge:openPath requires a file path");
    return forge.openLocalModel(path, mainWindow && !mainWindow.isDestroyed() ? mainWindow : null);
  });
  ...
  ipcMain.handle("ai:providerStatus", async () => {
    const { providerKeyStatus } = await import("./ai/secrets");
    return providerKeyStatus();
  });
  ipcMain.handle("ai:setProviderKey", async (_e, id: string, key: string) => {
    const { writeProviderKey, AI_PROVIDER_ACCOUNTS } = await import("./ai/secrets");
    if (!(id in AI_PROVIDER_ACCOUNTS)) throw new Error(`Unknown provider: ${id}`);
    await writeProviderKey(id as keyof typeof AI_PROVIDER_ACCOUNTS, key);
  });
  ipcMain.handle("ai:clearProviderKey", async (_e, id: string) => {
    const { clearProviderKey, AI_PROVIDER_ACCOUNTS } = await import("./ai/secrets");
    if (!(id in AI_PROVIDER_ACCOUNTS)) throw new Error(`Unknown provider: ${id}`);
    await clearProviderKey(id as keyof typeof AI_PROVIDER_ACCOUNTS);
  });
  ipcMain.handle("ai:probeProviders", async () => {
    const { probeProviders } = await import("./ai/providers");
    return probeProviders();
  });
  ipcMain.handle("ai:huggingfaceHealth", async () => {
    const { huggingfaceHealth } = await import("./ai/huggingface");
    return huggingfaceHealth();
  });
  ipcMain.handle("ai:getHfModel", async () => {
    const { getHfModel, HF_CODER_MODELS } = await import("./ai/huggingface");
    return { model: await getHfModel(), options: [...HF_CODER_MODELS] };
  });
  ipcMain.handle("ai:setHfModel", async (_e, model: string) => {
    const { setHfModel } = await import("./ai/huggingface");
    await setHfModel(model);
  });

  // GrudaChain — local AnythingLLM RAG + agentic R2/fleet tools
  ipcMain.handle("grudachain:health", async () => {
    const { anythingLlmHealth } = await import("./ai/anythingllm");
    return anythingLlmHealth();
  });
  ipcMain.handle("grudachain:workspaces", async () => {
    const { listWorkspaces } = await import("./ai/anythingllm");
    return listWorkspaces();
  });
  ipcMain.handle("grudachain:chat", async (_e, opts) => {
    const { grudachainChat } = await import("./ai/grudachainAgent");
    return grudachainChat(opts);
  });
  ipcMain.handle("grudachain:getConfig", async () => {
    const llm = await import("./ai/anythingllm");
    const [baseUrl, workspaceSlug, apiKey] = await Promise.all([
      llm.getAnythingLlmBaseUrl(),
      llm.getWorkspaceSlug(),
      llm.getAnythingLlmApiKey(),
    ]);
    return { baseUrl, workspaceSlug, hasApiKey: Boolean(apiKey) };
  });
  ipcMain.handle("grudachain:setConfig", async (_e, cfg: { baseUrl?: string; workspaceSlug?: string; apiKey?: string }) => {
    const llm = await import("./ai/anythingllm");
    if (cfg.baseUrl) await llm.setAnythingLlmBaseUrl(cfg.baseUrl);
    if (cfg.workspaceSlug) await llm.setWorkspaceSlug(cfg.workspaceSlug);
    if (cfg.apiKey) await llm.setAnythingLlmApiKey(cfg.apiKey);
  });
  ipcMain.handle("grudachain:clearApiKey", async () => {
    const { clearAnythingLlmApiKey } = await import("./ai/anythingllm");
    await clearAnythingLlmApiKey();
  });
  ...
  ipcMain.handle("coder:getPrefs", async () => {
    const ws = await workspaceStore.loadWorkspace();
    return {
      coderRoot: ws.coderRoot,
      coderProjectDir: ws.coderProjectDir,
      coderPort: ws.coderPort,
    };
  });
  ipcMain.handle("coder:setPrefs", async (_e, patch: { coderRoot?: string; coderProjectDir?: string; coderPort?: number }) =>
    workspaceStore.saveWorkspace(patch));
  ipcMain.handle("coder:pickRoot", async () => {
    const picked = await dialog.showOpenDialog(dialogParent() ?? (undefined as any), {
      title: "GrudachainCode IDE root (folder with package.json)",
      properties: ["openDirectory"],
    });
    if (picked.canceled || !picked.filePaths[0]) return null;
    return picked.filePaths[0];
  });
  ipcMain.handle("coder:pickProject", async () => {
    const picked = await dialog.showOpenDialog(dialogParent() ?? (undefined as any), {
      title: "Project workspace folder",
      properties: ["openDirectory"],
    });
    if (picked.canceled || !picked.filePaths[0]) return null;
    return picked.filePaths[0];
  ...
  // Grudge Engine — local The-ENGINE dev server + character viewer host
  ipcMain.handle("engine:launch", (_e, opts) => engine.launch(opts));
  ipcMain.handle("engine:stop", () => engine.stop());
  ipcMain.handle("engine:status", () => engine.getStatus());
  ipcMain.handle("engine:open", (_e, path?: string) => { engine.openInBrowser(path); });
  ipcMain.handle("engine:getPrefs", async () => {
    const ws = await workspaceStore.loadWorkspace();
    return {
      engineRoot: ws.engineRoot,
      enginePort: ws.enginePort,
      engineUseLocal: ws.engineUseLocal,
    };
  });
  ipcMain.handle("engine:setPrefs", async (_e, patch: {
    engineRoot?: string;
    enginePort?: number;
    engineUseLocal?: boolean;
  }) => workspaceStore.saveWorkspace(patch));
  ipcMain.handle("engine:pickRoot", async () => {
    const picked = await dialog.showOpenDialog(dialogParent() ?? (undefined as any), {
      title: "The-ENGINE repo root (folder with package.json + client/)",
      properties: ["openDirectory"],
    });
    if (picked.canceled || !picked.filePaths[0]) return null;
    return picked.filePaths[0];
  ...
  ipcMain.handle("fleet:games", async () => {
    try {
      ...
      return { static: FLEET_GAMES, live, merged: mergeFleetGames(FLEET_GAMES, live) };
    } catch {
