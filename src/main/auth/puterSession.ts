...
const KEYTAR_SERVICE = "grudge-dev-tool";
const FLEET_TOKEN_ACCOUNT = "default";
...
export async function setSession(puterToken: string, user: PuterUser): Promise<{ grudgeId: string }> {
  ...
  // Mirror Puter token into fleet API auth (list/search/upload-url) when no legacy bearer is set.
  try {
    const existing = await keytar.getPassword(KEYTAR_SERVICE, FLEET_TOKEN_ACCOUNT);
    if (!existing) await keytar.setPassword(KEYTAR_SERVICE, FLEET_TOKEN_ACCOUNT, puterToken);
  } catch { /* non-fatal */ }
  return { grudgeId };
}
