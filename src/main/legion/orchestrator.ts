...
export async function fetchObjectStoreCatalog(path: string): Promise<unknown> {
  ...
  // Normalize legacy `/api/v1/foo.json` → `/foo.json` for ONE TRUTH routing.
  p = p.replace(/^\/api\/v1/, "");
  const url = `${base}/api/objectstore/v1${p}`;
  const headers: Record<string, string> = { Accept: "application/json" };
  const fleetKey = await getFleetApiKey();
  if (fleetKey) headers.Authorization = `Bearer ${fleetKey}`;
  else {
    const token = await puterSession.getPuterToken();
    if (token) headers.Authorization = `Bearer ${token}`;
  }
