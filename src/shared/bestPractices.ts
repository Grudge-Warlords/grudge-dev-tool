...
export const PIPELINE_BEST_PRACTICES = {
  ...
  forgeHandoff: {
    ...
    rule: "Dev Tool / Pipeline: postMessage or deep-link import into forge.grudge-studio.com with CDN URL (not local blob for production). Scene save → R2 scenes/ + .meta.json; playtest via client/open.",
    category: "pipeline" as const,
...
export const SURFACE_BEST_PRACTICES = {
  ...
  dnsMap: {
    ...
    rule: "assets=R2 CDN · objectstore=catalogs/search · ai=Legion hub · forge=editor SPA · pipeline=grudge-pipeline.vercel.app · coder=coder.grudge-studio.com · id=auth · client=ONE TRUTH · obs=telemetry · water/open/character/grudox=games.",
    category: "surfaces" as const,
...
export const STUDIO_SURFACE_DNS = [
  ...
  { id: "forge", host: "forge.grudge-studio.com", role: "3D map/scene editor" },
