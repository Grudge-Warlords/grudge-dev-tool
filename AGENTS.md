...
|---|---|---|
| Local creation and editable regions | Existing creation / animation screens | 2026-09-05: six primitives and source-preserving local/catalog reuse; named/coloured effective and custom deformation regions with exact clip binding and bounded instructions. Four-view intake labels each file. Focused checks pass; full normal-app create/adjust/animate/save/reopen, kangaroo visual comparison and genuine four-view inference remain pending the coordinator desktop slot. Do not commit or declare production readiness before app acceptance. |
...
| Unified Prompt-to-3D router | `src/shared/prompt3dOrchestrator.ts` | One typed allowlisted owner surface ranks existing routes by quality/capability, verified readiness, provenance and resources. It shows blocked leaders and next eligible choices, never silently downgrades, and retains automatic recommendation/override/stages/exact revision in provenance. Deterministic routing contacts no model. Optional schema planning is explicit, loopback-only and prefers existing `grudge-dev`; no start, pull, install, external LLM or `localAgent.ts` fallback. The minimal scaffold is not final 4K visual acceptance; complete that design only with the user present. |
