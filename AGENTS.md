...
Both providers use the typed loopback job/installer service documented in [docs/prompt-to-3d.md](docs/prompt-to-3d.md). Generate must create and publish a retained job before preflight, show measured stage timings, and use bounded signed-manifest verification; full model-byte hashing stays in install/repair. Hunyuan must stop durably at `awaiting-concept-approval`; only the separate **Approve for 3D** action may bind the exact job, retained image SHA-256, current prompt/plan, seed, provider and AssetSpec before geometry loads. Technical framing checks are not semantic approval. Regeneration creates one concept with seed +1 and textures off while retaining immutable ancestry; stale or contradictory approval/provenance fails closed. Do not add automatic cloud fallback, Ollama mesh weights, global Python installs, or unpinned model downloads.
