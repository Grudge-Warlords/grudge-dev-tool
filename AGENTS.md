...
|---|---|---|
| Manual character/testing repairs | `creationPrompt.ts`, `proceduralCreation.ts`, `CreationFlowPage.tsx` | 2026-09-10: isolate new subject context, reject collapsed layouts, retain component orientation/colours, provide authored scale patterns and basic segmented idle, clear stale feedback. Local v1 package launched for Al; focused/build checks pass, real-model replay interrupted for manual control. Do not claim alligator visual or motion acceptance. See `docs/manual-testing-patches-20260910.md`. |
