...
const KEY_TO_ACCOUNT = {
  ...
  ANYTHINGLLM_API_KEY:          "grudachain.anythingllmApiKey",
  anythingllm_api_key:          "grudachain.anythingllmApiKey",
  ANYTHINGLLM_BASE_URL:         "grudachain.anythingllmBaseUrl",
  ANYTHINGLLM_WORKSPACE_SLUG:   "grudachain.workspaceSlug",
...
function resolveAccount(key, value, account) {
  if (key === "GEMINI_API_KEY" && /^sk-/i.test(value)) return "ai.openai";
  if (key === "CF_AIWORKER_API" && /^[a-f0-9]{32}$/i.test(value)) return "cf-account-id";
  return account;
}
...
async function main() {
  ...
  for (const rawLine of content.split(/\r?\n/)) {
    ...
    const resolved = resolveAccount(key, value, account);
    await keytar.setPassword(SERVICE, resolved, value);
    stored.push(resolved !== account ? `${key}→${resolved}` : key);
    if (key === "CLOUDFLARE_MAX_API" && value.startsWith("cfat_")) {
      await keytar.setPassword(SERVICE, "cf-ai-workers-api", value);
      stored.push("CLOUDFLARE_MAX_API→cf-ai-workers-api");
    }
  }
