...
async function main() {
  ...
  try {
    ...
    assert.ok(installerSource.includes("every model file SHA-256 verified"), "install/repair integrity must retain deep model snapshot verification");
    ...
    hardware.wsl.runtimeProbe = { distribution: "Ubuntu-24.04", nonRoot: true, homeWritable: true, osId: "ubuntu", osVersion: "24.04", python: "Python 3.12", cudaVisible: true, measuredAt: hardware.checkedAt, elapsedMs: 1 };
