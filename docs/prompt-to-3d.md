...
For Hunyuan, a technically valid concept also does not authorize geometry. It stops durably at **Awaiting explicit approval** and shows the exact retained image, current user prompt, compiled plan, seed, provider, AssetSpec revision, image hash, technical review and a separate manual semantic checklist. Only **Approve for 3D** may bind that exact state and continue after a fresh pre-run check. **Regenerate concept with new seed** creates one concept, advances the seed exactly once, keeps textures off and preserves immutable attempt history; **Edit prompt** creates a separately linked attempt only after the brief changes.

The root/variant AssetSpec records, `concept-prompt.json`, `concept-approval.json`, saved job and final provenance use one authoritative current prompt/plan/seed. Approval records the explicit action time/source and binds the job, attempt, concept SHA-256, prompt, seed, provider and canonical AssetSpec fingerprint. Legacy `originalPrompt` or concept-job reuse cannot authorize geometry. Missing, stale, changed or contradictory approval/provenance fails closed.
